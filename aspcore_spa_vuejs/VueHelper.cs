...
namespace aspcore_spa_vuejs
{
  public static class VueHelper
  {
    // default port number of 'yarn run serve'
    ...
    // done message of 'yarn run serve' command.
    ...
    public static void UseVueDevelopmentServer(this ISpaBuilder spa)
    {
      spa.UseProxyToSpaDevelopmentServer(async () =>
      {
        ...
        var logger = loggerFactory.CreateLogger("Vue");
              // if 'yarn run serve' command was executed yourself, then just return the endpoint.
              if (IsRunning())
        {
        ...
        var processInfo = new ProcessStartInfo
        {
          ...
          Arguments = $"{(isWindows ? "/c yarn " : "")}run serve",
        ...
        _ = Task.Run(() =>
            ...
            catch (EndOfStreamException ex)
            {
              ...
              tcs.SetException(new InvalidOperationException("'yarn run serve' failed.", ex));
            }
          });
        _ = Task.Run(() =>
              {
